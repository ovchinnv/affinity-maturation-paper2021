...
qhprof = 0 ;% 1 = activated proliferation (KP93) ; 
% param below are reasonable starting poits for non-activated model: 
q=1.1;
k_prof = q * 1. * (1+qavid); % /day ; rate of B cell proliferation at maximum stimulation ;
k_death= q * 7.5 / (1+qavid); % /day ; rate of B cell death ;
%
...
if (~exist('hcexp'))
% hcexp=0.94; %exponent of activation function, which is hc or hct, depending on qtcell (1 is the default that corresponds to KP93)
 hcexp=0.715; %exponent of activation function, which is hc or hct, depending on qtcell (1 is the default that corresponds to KP93)
end
%
if (~exist('hcscale'))
 hcscale=0.946 ; % scaling for the activation function h
end
