...
if (1) % print distributions
clf ; hold on ; box on ;
lw=1.5;
semilogx(affinity,ibc0,'g-*', 'linewidth', lw);
%semilogx(affinity,ibc0,'r*');
semilogx(affinity,ibc1,'b--o', 'linewidth', lw);
xlabel('\it Affinity(nondimensional)')
ylabel('\it Concentration(nondimensional)');
set(gca, 'fontsize', 15)
l=legend('original (BCRs #2-3)','shifted (BCR #1)') ; legend boxoff
set(l, 'orientation', 'horizontal')
ylim([0 0.0017])
text(1e-3, 0.0018, 'D','fontsize',17)
set(gcf, 'paperpositionmode','auto')
print(gcf, '-dpng', 'dist.png')
print(gcf, '-depsc2', 'dist.eps')
end
